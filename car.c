...
// Defination of addAdmin() Function
void addAdmin(){

    // Open a admin.bin file in rb mode
    FILE *fp = fopen("admin.bin","rb");

    // Check Weather file is open or not
    if(fp == NULL){

        // Now Open file admin.bin in wb mode
        fp = fopen("admin.bin","wb");
        // Making array of Stucture of User and it's size is 2
        User u[2] = {{"admin","abc","lok"},{"super","abc","loken"}};
        // Now Write User array content in emp.bin file
        fwrite(u,sizeof(u),1,fp);
    }

    // finally close emp file for save data in this file
    fclose(fp);
}

// Defination of getInput() function which have "return type" is "User Pointer"
User * getInput(){
    // Declaring Variable
    int i;
    // Create Static Variable of User Structure Type , that's why we can return it
    // ****** Remember When we want to return address of variable or structure
    // then we use STATIC in front of that variable because
    // when function is end than , Whole variable of this function is destroyed by compiler
    // There is concept of dangling pointer
    // But When we use static the variable is live till the end of program
    static User u;
    // clear screen
    clrscr();
    // Now we shift our cursor to 32 column and 1 row
    gotoxy(32,1);
    // We are Changing our text color in output screen
    textcolor(YELLOW);
    printf("CAR RENTAL SYSTEM\n");
    // Run for loop 80 time to create a line in output screen
    for(i=1;i<=80;i++){
        printf("%c",247);
    }
    // Again shift our cursor at 32 col and 4 row
    gotoxy(32,4);
    printf("* LOGIN PANEL *");
    gotoxy(1,7);
    textcolor(LIGHTCYAN);
    ...
    // we are cleaning the buffer area
    fflush(stdin);
    textcolor(WHITE);
    // Remember --> fgets is used to take multiple words with control input
    fgets(u.userid,20,stdin);
    // pos is pointer of char type
    char *pos;
    // Remember --> strchr is use to search a character in given string
    // it takes two input 1. charater_array , 2. The word which we want to search in string
    // it return --> either address of character or NULL
    // if it find character in given string then it return character's address
    // if it didn't found character then it return Null

    pos = strchr(u.userid,'\n');
    // check what kind of value carry by pos after strchr function
    if(pos != NULL){
        // We are replaceing  '\n' by '\0' in given string in strchr() function
        *pos = '\0';
    }

    // if user entered 0 that's means he/she want to cancel the login
    ...
    // we are take password form user in form HIDDEN character
    gotoxy(25,11);
    textcolor(LIGHTCYAN);
    ...
    for(;;){
        ...
        // if check Weather user enter "ENTER" or NOT
        if(u.pwd[i] == 13){
            u.pwd[i] = '\0';
            break;
        }

        // if check Weather user enter "BACKSPACE" or NOT
        if(u.pwd[i]==8){
            i-=1;
            printf("\b \b");
            continue;
    ...
    // TESTING --> printf("%s",u.pwd);

    // It check , Is user want to cancel Login
    if((strcmp(u.pwd,"0"))==0){
        textcolor(LIGHTRED);
        gotoxy(30,17);
        printf("Login Cancelled !");
...
// This check Weather user is exist or not which is given as input
// Remember --> with 2nd argument we can decide wheather we have to check admin detail or emp deatil
int checkUserExist(User u,char * usertype){

    // Here we handle if user enter only empty string
    if(strlen(u.userid)==0 || strlen(u.pwd)==0){
        ...
        getch();
        gotoxy(28,20);
        // This is technique by which we can erase certain part of output screen
        printf("\t\t\t\t\t\t");
        return 0;
    }

    // if string is not empty then user input something , Now we deal with it
    FILE *fp;

    // This if decides that whose detail we have to check (admin or emp)
    ...
    // with the help of found variable we decide recode is match or not
    int found = 0;
    User user;

    // this loop read recodes one by one and check userid with every recode
    while(fread(&user,sizeof(user),1,fp) == 1){
    //  printf("Id -> '%s'   Pwd -> '%s'\n",user.userid,user.pwd);
//      printf("Id -> '%s'   Pwd -> '%s'\n",u.userid,u.pwd);
        //getch();
...
//This function show all functionality of admin
int adminMenu(){
    int i,choice;
    textcolor(LIGHTRED);
    gotoxy(32,2);
...
// This funtion add Employ which done by Admin
void addEmployee()
{
    char empid[10] ={"EMP-"};
    char temp[20];
...
// this function simply add a car
void addCarDetails()
{
    ...
    do{
        ...
        // <--- Messege of Successfully add car --->
        textcolor(LIGHTGREEN);
        gotoxy(30,15);
        ...
        // if you remove record in write in file
        fseek(fp,0,SEEK_END);
...
int rentCar()
{
    ...
    int valid = validDate(CC.sd,CC.ed);
    if(valid != 1)
    {
        while(valid != 1)
        {
...
// <--- This Funtion Compare Current-date , start-date and end-date --->
...
// <--- This Function Increment Car Count , When Car is Returned --->
...
// <--- This Function Check Weather Current_date is equal to Return_Car_Date OR NOT
void return_car()
{
    FILE * fp = fopen("customer.bin","rb");
    if(fp == NULL)
...
// <--- This Funciton Simply Delete Customer-Record From "Customer.bin" When Car is Returned
void return_car_delete()
{
    ...
    if(found == 0)
    {
        ...
        textcolor(LIGHTGREEN);
        gotoxy(30,10);
        printf("Car ID     :  %d",cc.car_id);
        gotoxy(30,12);
        printf("Car Model  :  %s",getCarName(cc.car_id));
        gotoxy(1,20);
        textcolor(WHITE);
        printf("Press any key to Continue ");
        getch();
